/* LedControl
   Pin 12 -> First MAX7219 DATA IN (GRAY)
   Pin 11 -> First MAX7219 CLK (BLUE)
   Pin 10 -> First MAX7219 CS (PURPLE)
   2 x Max7219
   Connect from 1ST Max7219 OUT to 2ND Max7219 IN
*/
#include <LedControl.h>
LedControl field = LedControl(12, 11, 10, 2);
...
void loop() {
  ...
  while ((p1Score < 9) && (p2Score < 9)) {
    ...
    //        Serial.println(" D");
    //      }
    //      Serial.print("game pace:");
    //      Serial.println(gamePace);
...
void displayPlayers() {
  ...
  //display strikers
  field.setLed(0, 4, p2Left, true); //p1 striker
  field.setLed(0, 4, p2Right, true); //p1 striker

  field.setLed(1, 3, p1Left, true); //p2 striker
  field.setLed(1, 3, p1Right, true); //p2 striker
}
...
void evaluatePlay() {
  ...
  if ((x == 3) && (fieldId == 0) && (rightDir)) { //ball in front of striker 2
    if ((y == p2Left) or (y == p2Right)) {  //ball is right in front of the striker
      //Serial.println("p2 striker blocked the ball");
      rightDir = false;  //change direction
    } else if (((y == (p2Right + 1)) && (!upDir)) or ((y == (p2Left - 1)) && (upDir))) { //TODO: combine 2 and 3 below
      //Serial.println("p2 striker blocked the ball - diagonal");
      rightDir = false;  //change direction
      upDir = !upDir;
    }
  }
  ...
  if ((x == 4) && (fieldId == 0) && (!rightDir) && ((y == p2Left) or (y == p2Right))) { //ball ON striker 2
    upDir = !upDir;
  }

  //ball on p1 striker so deflect the ball
  if ((x == 3) && (fieldId == 1) && (rightDir) && ((y == p1Left) or (y == p1Right))) { //ball on striker 1
    upDir = !upDir;
  }

  //ball on front of p1 striker
  if ((x == 4) && (fieldId == 1) && (!rightDir)) { //front of striker 1
    if ((y == p1Left) or (y == p1Right)) {  //ball is right in front of the striker
      //Serial.println("p1 striker blocked the ball");
      rightDir = true;  //change direction
    } else if (((y == (p1Right - 1)) && (upDir)) or ((y == (p1Left + 1)) && (!upDir))) {
      //Serial.println("p1 striker blocked the ball - diagonal");
      rightDir = true;  //change direction
      upDir = !upDir;
    }
  }
